...
function ready(error, us, DataExplorationUtility) {
  ...
  DataExplorationUtility.forEach(function(d) {
	rateById[d.id] = +d.TotalHotelSales;
  });
  var state = {};
  DataExplorationUtility.forEach(function(d){
  ...
  var revCount = {};
  DataExplorationUtility.forEach(function(d){
	  revCount[d.id] = d.TotalHotelSales;
  });
  var revMin = {};
  DataExplorationUtility.forEach(function(d){
      ...
      .attr("class", "counties")
			...
			.on("mouseover", function(d){
				if (typeof(revCount[d.id]) != "undefined"){
					document.getElementById("hotelCount").innerHTML = "State: " + state[d.id] + "\tTotal Hotels:" + revCount[d.id] + "\tMinimum Rating: " + revMin[d.id] + "\tMaximum Rating: " + revMax[d.id] + "\tAverage Rating: " + revAvg[d.id].toFixed(2);;			
				}
				else{
					document.getElementById("hotelCount").innerHTML = "State: N/A\tTotal Hotels: 0\tMinimum Rating: 0\tMaximum Rating: 0\tAverage Rating: 0";		
				}
